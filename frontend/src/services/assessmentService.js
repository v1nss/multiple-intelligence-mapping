...
export const assessmentService = {
  ...
  async getQuestions(assessmentId) {
    const res = await api.get(`/api/assessments/${assessmentId}/questions`);
    return res.data;
  },

  async submitAssessment(assessmentId, responses) {
    const res = await api.post(`/api/assessments/${assessmentId}/submit`, { responses });
    return res.data;
  },

  async getResult(assessmentId) {
    const res = await api.get(`/api/assessments/${assessmentId}/result`);
    return res.data;
  },

  async getHistory() {
    const res = await api.get('/api/assessments/history');
    return res.data;
  },
